...
class SamplingParams:
    ...
    def __init__(
        ...
        loop_window_size: int = 256,
        loop_penalty: float = 0.0,
        loop_repeat_threshold: int = 2,
        loop_min_output_tokens: int = 64,
        loop_hard_block: bool = False,
        loop_observe_only: bool = False,
        ...
        self.loop_ngram_size = loop_ngram_size if loop_ngram_size is not None else 0
        self.loop_window_size = (
            loop_window_size if loop_window_size is not None else 256
        )
        self.loop_penalty = loop_penalty if loop_penalty is not None else 0.0
        self.loop_repeat_threshold = (
            loop_repeat_threshold if loop_repeat_threshold is not None else 2
        )
        self.loop_min_output_tokens = (
            loop_min_output_tokens if loop_min_output_tokens is not None else 64
        )
        self.loop_hard_block = loop_hard_block if loop_hard_block is not None else False
        self.loop_observe_only = (
            loop_observe_only if loop_observe_only is not None else False
        )
        self.loop_debug = loop_debug if loop_debug is not None else False
    ...
    def verify(self, vocab_size):
        ...
        if self.loop_ngram_size < 0 or self.loop_ngram_size == 1:
            raise ValueError(
                "loop_ngram_size must be 0 (disabled) or at least 2, "
                f"got {self.loop_ngram_size}."
            )
        if self.loop_ngram_size > 0:
            if self.loop_penalty < 0.0:
                raise ValueError(
                    "loop_penalty must be non-negative when loop_ngram_size is "
                    f"enabled, got {self.loop_penalty}."
                )
            if not self.loop_observe_only and self.loop_penalty <= 0.0:
                raise ValueError(
                    "loop_penalty must be positive when loop_ngram_size is enabled, "
                    f"got {self.loop_penalty}."
                )
            if self.loop_window_size < self.loop_ngram_size:
                raise ValueError(
                    "loop_window_size must be at least loop_ngram_size when "
                    f"loop_ngram_size is enabled, got {self.loop_window_size}."
                )
            if self.loop_repeat_threshold < 1:
                raise ValueError(
                    "loop_repeat_threshold must be at least 1 when "
                    f"loop_ngram_size is enabled, got {self.loop_repeat_threshold}."
                )
            if self.loop_min_output_tokens < 0:
                raise ValueError(
                    "loop_min_output_tokens must be non-negative, got "
                    f"{self.loop_min_output_tokens}."
                )
