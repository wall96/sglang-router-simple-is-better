...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def create_ba_proj(
        ...
        return MergedColumnParallelLinear(
            ...
            bias=False,
            # The checkpoint stores in_proj_a and in_proj_b as unquantized
            # tensors. Runtime fuses them as in_proj_ba, so keep the fused
            # layer unquantized as well.
            quant_config=None,
            prefix=prefix,
            tp_rank=tp_rank,
            tp_size=tp_size,
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                name = name.replace(weight_name, param_name)
                if (
                    (name.endswith(".weight_scale") or name.endswith("_weight_scale"))
                    and name not in params_dict
                    and f"{name}_inv" in params_dict
                ):
                    name = f"{name}_inv"
                # Skip loading extra parameters for GPTQ/modelopt models.
                ...
                for mapping in expert_params_mapping:
                    ...
                    if (
                        (
                            name_mapped.endswith(".weight_scale")
                            or name_mapped.endswith("_weight_scale")
                        )
                        and name_mapped not in params_dict
                        and f"{name_mapped}_inv" in params_dict
                    ):
                        name_mapped = f"{name_mapped}_inv"
                    if is_fused_expert:
