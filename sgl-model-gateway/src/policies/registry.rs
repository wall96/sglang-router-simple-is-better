...
use super::{BucketPolicy, CacheAwarePolicy, LMetricPolicy, LoadBalancingPolicy, PolicyFactory};
...
impl PolicyRegistry {
    ...
    fn create_policy_from_type(&self, policy_type: &str) -> Arc<dyn LoadBalancingPolicy> {
        if policy_type == "cache_aware" {
            ...
            Arc::new(cache_aware)
        } else if policy_type == "lmetric" {
            let mut lmetric = LMetricPolicy::new();
            let mesh_sync = &*self.mesh_sync.read().unwrap();
            lmetric.set_mesh_sync(mesh_sync.clone());
            Arc::new(lmetric)
        } else {
    ...
    fn init_tree_backed_policy(policy: &Arc<dyn LoadBalancingPolicy>, workers: &[Arc<dyn Worker>]) {
        match policy.name() {
            "cache_aware" => {
            ...
            "lmetric" => {
                if let Some(lmetric) = policy.as_any().downcast_ref::<LMetricPolicy>() {
                    lmetric.init_workers(workers);
                }
            }
            _ => {}
        }
    ...
    fn remove_worker_from_tree_backed_policy(
    ...
    fn remove_worker_url_from_tree_backed_policy(
        policy: &Arc<dyn LoadBalancingPolicy>,
        worker_url: &str,
    ) {
        match policy.name() {
            ...
            "lmetric" => {
                if let Some(lmetric) = policy.as_any().downcast_ref::<LMetricPolicy>() {
                    lmetric.remove_worker_by_url(worker_url);
                }
            }
            _ => {}
        }
    ...
    fn apply_remote_tree_operation_to_policy(
        policy: &Arc<dyn LoadBalancingPolicy>,
        model_id: &str,
        ...
        match policy.name() {
            ...
            "lmetric" => {
                if let Some(lmetric) = policy.as_any().downcast_ref::<LMetricPolicy>() {
                    lmetric.apply_remote_tree_operation(model_id, operation);
                }
            }
            _ => {}
        }
    ...
    pub fn init_cache_aware_policy(&self, model_id: &str, workers: &[Arc<dyn Worker>]) {
        ...
        if let Some(policy) = self.get_policy(model_id) {
            if matches!(policy.name(), "cache_aware" | "lmetric") {
                debug!(
                    "Initializing {} policy with {} workers for model {}",
                    policy.name(),
                    workers.len(),
                    model_id
                );
                Self::init_tree_backed_policy(&policy, workers);
            }
        }
    }
    ...
    pub fn remove_worker_from_cache_aware(&self, model_id: &str, worker_url: &str) {
        ...
        if let Some(policy) = self.get_policy(model_id) {
            if matches!(policy.name(), "cache_aware" | "lmetric") {
                Self::remove_worker_url_from_tree_backed_policy(&policy, worker_url);
                debug!(
                    "Removed worker {} from {} policy for model {}",
                    worker_url,
                    policy.name(),
                    model_id
                );
    ...
    pub fn init_pd_cache_aware_policies(
        ...
        if let Some(prefill_policy) = self.prefill_policy.get() {
            if matches!(prefill_policy.name(), "cache_aware" | "lmetric") {
                if !prefill_workers.is_empty() {
                    debug!(
                        "Initializing prefill {} policy with {} workers",
                        prefill_policy.name(),
                        prefill_workers.len()
                    );
                    Self::init_tree_backed_policy(prefill_policy, prefill_workers);
                }
            }
        }
        ...
        if let Some(decode_policy) = self.decode_policy.get() {
            if matches!(decode_policy.name(), "cache_aware" | "lmetric") {
                if !decode_workers.is_empty() {
                    debug!(
                        "Initializing decode {} policy with {} workers",
                        decode_policy.name(),
                        decode_workers.len()
                    );
                    Self::init_tree_backed_policy(decode_policy, decode_workers);
                }
            }
        }
    }
    ...
    pub fn remove_pd_worker_from_cache_aware(&self, worker: &dyn Worker) {
        ...
        if let Some(policy) = policy {
            if matches!(policy.name(), "cache_aware" | "lmetric") {
                Self::remove_worker_from_tree_backed_policy(policy, worker);
                debug!(
                    "Removed PD worker {} ({}) from {} policy",
                    worker.url(),
                    worker.worker_type(),
                    policy.name()
                );
    ...
    pub fn apply_remote_tree_operation(
        ...
        if let Some(policy) = self.get_policy(model_id) {
            Self::apply_remote_tree_operation_to_policy(&policy, model_id, operation);
        }

        // Also check default policy if it's cache-aware
        Self::apply_remote_tree_operation_to_policy(&self.default_policy, model_id, operation);

        // Check prefill and decode policies for PD mode
        if let Some(prefill_policy) = self.prefill_policy.get() {
            Self::apply_remote_tree_operation_to_policy(prefill_policy, model_id, operation);
        }

        if let Some(decode_policy) = self.decode_policy.get() {
            Self::apply_remote_tree_operation_to_policy(decode_policy, model_id, operation);
        }
    }
}
