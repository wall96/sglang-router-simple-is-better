...
impl<D: WorkerRegistrationData + WorkflowData> StepExecutor<D> for UpdatePoliciesStep {
    async fn execute(&self, context: &mut WorkflowContext<D>) -> WorkflowResult<StepResult> {
        ...
        // Initialize tree-backed policies for PD mode (prefill_policy / decode_policy
        ...
        let prefill_is_tree_backed = matches!(prefill_policy.name(), "cache_aware" | "lmetric");
        let decode_is_tree_backed = matches!(decode_policy.name(), "cache_aware" | "lmetric");
        if (prefill_is_tree_backed && !prefill_workers.is_empty())
            || (decode_is_tree_backed && !decode_workers.is_empty())
        {
            app_context
                .policy_registry
