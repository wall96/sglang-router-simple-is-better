...
pub trait Worker: Send + Sync + fmt::Debug {
    ...
    /// Sum of not-yet-drained prefill tokens for in-flight requests routed
    /// to this worker (the "queued prefill tokens" component of LMETRIC's
    /// P-token indicator). Default 0 for workers/policies that don't track it.
    fn queued_prefill_tokens(&self) -> usize {
        0
    }

    /// Record `n` new (uncached) prefill tokens for a request just admitted
    /// to this worker. Paired 1:1 with a later [`Worker::pop_queued_prefill_tokens`]
    /// when that request completes. Default no-op.
    fn push_queued_prefill_tokens(&self, _n: usize) {}

    /// Drain one previously-pushed prefill-token amount (FIFO) when a request
    /// on this worker completes. Only the aggregate sum is consumed by the
    /// scheduler, so FIFO order keeps the running sum exact regardless of which
    /// specific request finished. Default no-op.
    fn pop_queued_prefill_tokens(&self) {}
...
pub struct BasicWorker {
    ...
    /// Fast-read running sum of queued (in-flight, not-yet-drained) prefill
    /// tokens — the queued component of LMETRIC's P-token indicator.
    pub queued_prefill_counter: Arc<AtomicUsize>,
    /// FIFO of per-request prefill-token amounts backing `queued_prefill_counter`.
    /// Pushed on admission, popped 1:1 on request completion.
    pub queued_prefill_backlog: Arc<StdMutex<VecDeque<usize>>>,
...
impl Worker for BasicWorker {
    ...
    fn queued_prefill_tokens(&self) -> usize {
        self.queued_prefill_counter.load(Ordering::Relaxed)
    }

    fn push_queued_prefill_tokens(&self, n: usize) {
        // Hold the backlog lock while bumping the sum so the two stay consistent.
        let mut backlog = self.queued_prefill_backlog.lock().unwrap();
        backlog.push_back(n);
        self.queued_prefill_counter.fetch_add(n, Ordering::Relaxed);
    }

    fn pop_queued_prefill_tokens(&self) {
        let mut backlog = self.queued_prefill_backlog.lock().unwrap();
        if let Some(n) = backlog.pop_front() {
            self.queued_prefill_counter.fetch_sub(n, Ordering::Relaxed);
        }
    }
...
impl Worker for DPAwareWorker {
    ...
    fn queued_prefill_tokens(&self) -> usize {
        self.base_worker.queued_prefill_tokens()
    }

    fn push_queued_prefill_tokens(&self, n: usize) {
        self.base_worker.push_queued_prefill_tokens(n);
    }

    fn pop_queued_prefill_tokens(&self) {
        self.base_worker.pop_queued_prefill_tokens();
    }
...
impl Drop for WorkerLoadGuard {
    fn drop(&mut self) {
        ...
        // Drain this request's queued-prefill contribution (no-op for policies
        // that never pushed, e.g. cache_aware/manual — their backlog is empty).
        self.worker.pop_queued_prefill_tokens();
