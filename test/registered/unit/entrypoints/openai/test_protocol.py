...
class TestCompletionRequest(unittest.TestCase):
    ...
    def test_completion_request_sglang_extensions(self):
        ...
        self.assertEqual(request.loop_ngram_size, 5)
        self.assertEqual(request.loop_window_size, 256)
        self.assertEqual(request.loop_penalty, 6.0)
        self.assertEqual(request.loop_repeat_threshold, 2)
        self.assertEqual(request.loop_min_output_tokens, 64)
        self.assertFalse(request.loop_hard_block)
        self.assertTrue(request.loop_observe_only)
        self.assertTrue(request.loop_debug)
