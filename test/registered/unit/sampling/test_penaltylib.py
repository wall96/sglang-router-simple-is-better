...
from sglang.srt.sampling.penaltylib.loop_penalty import BatchedLoopPenalizer
...
def _make_req(
    ...
    eos_id=2,
    loop_ngram_size=0,
    loop_window_size=256,
    loop_penalty=0.0,
    loop_repeat_threshold=2,
    loop_min_output_tokens=64,
    loop_hard_block=False,
    loop_observe_only=False,
    loop_debug=False,
):
    ...
    req.sampling_params.loop_ngram_size = loop_ngram_size
    req.sampling_params.loop_window_size = loop_window_size
    req.sampling_params.loop_penalty = loop_penalty
    req.sampling_params.loop_repeat_threshold = loop_repeat_threshold
    req.sampling_params.loop_min_output_tokens = loop_min_output_tokens
    req.sampling_params.loop_hard_block = loop_hard_block
    req.sampling_params.loop_observe_only = loop_observe_only
    req.sampling_params.loop_debug = loop_debug
...
class TestBatchedLoopPenalizer(CustomTestCase):

    def _setup(self, configs):
        """configs: list of loop-penalty kwargs."""
        reqs = [_make_req(**config) for config in configs]
        batch = _make_batch(reqs)
        orch = BatchedPenalizerOrchestrator(
            VOCAB_SIZE, batch, {BatchedLoopPenalizer}
        )
        pen = orch.penalizers[BatchedLoopPenalizer]
        return orch, pen

    def _feed(self, pen, token_ids):
        for token_id in token_ids:
            pen.cumulate_output_tokens(torch.tensor([token_id]))

    def test_is_required_with_enabled_loop_penalty(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
                    "loop_penalty": 4.0,
                    "loop_min_output_tokens": 0,
                }
            ]
        )
        self.assertTrue(pen.is_required())

    def test_is_not_required_when_disabled(self):
        _, pen = self._setup([{}])
        self.assertFalse(pen.is_required())

    def test_penalizes_repeated_ngram_continuation(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
                    "loop_window_size": 100,
                    "loop_penalty": 4.0,
                    "loop_repeat_threshold": 2,
                    "loop_min_output_tokens": 0,
                }
        ...
        self._feed(pen, [1, 2, 3, 4, 1, 2, 3, 1, 2])

        logits = torch.zeros(1, VOCAB_SIZE)
        pen.apply(logits)
        self.assertAlmostEqual(logits[0, 3].item(), -4.0, places=5)
        self.assertAlmostEqual(logits[0, 4].item(), 0.0, places=5)

    def test_threshold_allows_early_repetition(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
    ...
    def test_hard_block_sets_negative_infinity(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
        ...
        logits = torch.zeros(1, VOCAB_SIZE)
        pen.apply(logits)
        self.assertTrue(torch.isinf(logits[0, 3]) and logits[0, 3] < 0)

    def test_observe_only_records_hit_without_changing_logits(self):
        req = _make_req(
            loop_ngram_size=3,
            loop_window_size=100,
            loop_penalty=0.0,
        ...
        pen.apply(logits)
        self.assertTrue(torch.equal(logits, original))
        self.assertEqual(req.loop_penalty_stats["hit_count"], 1)
        self.assertEqual(req.loop_penalty_stats["penalized_token_count"], 1)
        self.assertEqual(req.loop_penalty_stats["last_token_ids"], [3])
        self.assertTrue(req.loop_penalty_stats["observe_only"])

    def test_observe_only_deduplicates_same_output_length_stats(self):
        req = _make_req(
            loop_ngram_size=3,
            loop_window_size=100,
            loop_penalty=0.0,
    ...
    def test_min_output_tokens_delays_penalty(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
    ...
    def test_prefers_request_output_ids_over_internal_history(self):
        req = _make_req(
            loop_ngram_size=3,
            loop_window_size=100,
            loop_penalty=4.0,
        ...
        req.output_ids = [1, 2, 3, 4, 1, 2, 3, 1, 2]
        batch = _make_batch([req])
        orch = BatchedPenalizerOrchestrator(
            VOCAB_SIZE, batch, {BatchedLoopPenalizer}
        )
    ...
    def test_filter_keeps_loop_state(self):
        _, pen = self._setup(
            [
                {
                    "loop_ngram_size": 3,
        ...
        for token_id in [1, 2, 3, 4, 1, 2, 3, 1, 2]:
            pen.cumulate_output_tokens(torch.tensor([token_id, token_id]))

        pen.filter(torch.tensor([1]))
        logits = torch.zeros(1, VOCAB_SIZE)
        pen.apply(logits)
        self.assertAlmostEqual(logits[0, 3].item(), -5.0, places=5)

    def test_merge_concatenates_loop_state(self):
        _, pen1 = self._setup(
            [
                {
                    "loop_ngram_size": 3,
        ...
        pen1.merge(pen2)
        self.assertEqual(len(pen1.histories), 2)
        self.assertEqual(pen1.penalties, [2.0, 3.0])

    def test_teardown_cleans_attributes(self):
        _, pen = self._setup(
            [{"loop_ngram_size": 3, "loop_penalty": 2.0, "loop_min_output_tokens": 0}]
        )
        pen.teardown()
        self.assertFalse(hasattr(pen, "histories"))
        self.assertFalse(hasattr(pen, "penalties"))
