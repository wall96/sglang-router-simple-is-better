...
class TestSamplingParamsVerify(CustomTestCase):
    ...
    # --- loop penalty ---
    def test_loop_penalty_defaults_disabled(self):
        """Default loop penalty params should be disabled and valid."""
        sp = self._make()
        self.assertEqual(sp.loop_ngram_size, 0)
        self.assertEqual(sp.loop_penalty, 0.0)
        self.assertFalse(sp.loop_observe_only)
        self.assertFalse(sp.loop_debug)
        sp.verify(self.VOCAB_SIZE)

    def test_loop_penalty_enabled_valid(self):
        """Test that valid loop penalty params pass verify()."""
        sp = self._make(
            loop_ngram_size=5,
    ...
    def test_loop_ngram_size_one_raises(self):
        """loop_ngram_size=1 is too broad and should be rejected."""
        sp = self._make(loop_ngram_size=1, loop_penalty=1.0)
        with self.assertRaises(ValueError):
            sp.verify(self.VOCAB_SIZE)

    def test_loop_penalty_required_when_enabled(self):
        """Enabling loop n-grams requires a positive additive penalty."""
        sp = self._make(loop_ngram_size=5, loop_penalty=0.0)
        with self.assertRaises(ValueError):
            sp.verify(self.VOCAB_SIZE)

    def test_loop_observe_only_allows_zero_penalty(self):
        """Observe-only loop detection does not need a positive additive penalty."""
        sp = self._make(
            loop_ngram_size=5,
            loop_penalty=0.0,
    ...
    def test_loop_penalty_cannot_be_negative(self):
        """Negative loop penalty is invalid even in observe-only mode."""
        sp = self._make(
            loop_ngram_size=5, loop_penalty=-1.0, loop_observe_only=True
        )
        with self.assertRaises(ValueError):
            sp.verify(self.VOCAB_SIZE)

    def test_loop_window_must_cover_ngram(self):
        """Window smaller than the n-gram cannot detect a full n-gram."""
        sp = self._make(loop_ngram_size=5, loop_window_size=4, loop_penalty=1.0)
        with self.assertRaises(ValueError):
            sp.verify(self.VOCAB_SIZE)

    def test_loop_repeat_threshold_must_be_positive(self):
        """Loop repeat threshold must be at least one."""
        sp = self._make(
            loop_ngram_size=5, loop_penalty=1.0, loop_repeat_threshold=0
        )
        with self.assertRaises(ValueError):
            sp.verify(self.VOCAB_SIZE)
